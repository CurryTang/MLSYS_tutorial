...
面试中回答该题可遵循**“单树训练与分裂机制 $\to$ 随机森林双重随机性 $\to$ 偏差-方差理论对比 $\to$ 落地权衡与工程考量 $\to$ 高频追问深潜”**的逻辑展开：

```mermaid
flowchart TD
    A["单决策树 (High Variance, Low Bias)"] --> B["CART 贪心递归二分<br>Gini / 交叉熵 / 方差缩减<br>代价复杂度剪枝"]
    B --> C{"致命痛点: 层级误差累积与不稳定性"}
    C --> D["双重随机化破局 (Random Forest)"]
    D --> E["行采样: Bootstrap 抽样 (36.8% OOB 验证)"]
    D --> F["列采样: 节点候选子空间 (m ≈ √p / p/3)"]
    E & F --> G["方差削减定理: Var = ρσ² + (1-ρ)σ²/B"]
    G --> H["压低树间相关系数 ρ，总方差断崖式下降"]
    H --> I["工程权衡: 白盒可合规 (单树) vs 稳健工业 Baseline (随机森林)"]
```
...
```mermaid
flowchart TD
    Start["新业务建模任务"] --> Q1{"业务是否要求向审计/监管机构<br>出具确定性、无争议的 if-else 规则链？<br>(如信贷审批、风控拒贷准入、医疗诊断)"}
    Q1 -- "是 (合规刚性)" --> Tree["强制选择单决策树<br>+ 代价复杂度剪枝 (CART)<br>确保逻辑清晰可审计"]
    Q1 -- "否" --> Q2{"运行环境是否在嵌入式/微控制器 (MCU)<br>或在线打分延迟要求严格 < 0.1 ms？"}
    Q2 -- "是 (资源严苛)" --> Tree
    Q2 -- "否" --> Q3{"追求稳健开箱即用泛化性能，<br>且团队算力支持并行训练？"}
    Q3 -- "是" --> RF["首选随机森林 (Random Forest)<br>• 免繁琐超参微调<br>• 利用 OOB 评估与 MDA 特征筛选"]
```
